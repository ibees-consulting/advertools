            ...
            try:
                detail_df = pd.concat(
                    [
                        detail_df,
                        pd.DataFrame(
                            [x[detail] for x in video_resp.json()["items"]]
                        ),
                    ],
                    axis=1,
                )
            ...
            try:
                detail_df = pd.concat(
                    [
                        detail_df,
                        pd.DataFrame(
                            [x[detail] for x in channel_resp.json()["items"]]
                        ),
                    ],
                    axis=1,
                )
...
def serp_goog(
    ...
    * "tickets" in Australia
    ...
    ordered_cols = (
        list(set(params_list[i]).difference({"q", "key", "cx"}))
        + specified_cols
    )
    ...
    if "pagemap" in final_df:
        ...
        for p in final_df["pagemap"]:
            try:
                ...
                pagemap_df = pd.concat(
                    [pagemap_df, temp_pagemap_df], sort=False
                )
            except Exception:
                temp_pagemap_df = pd.DataFrame(
                    {"delete_me": None}, index=range(1)
                )
                pagemap_df = pd.concat(
                    [pagemap_df, temp_pagemap_df], sort=False
                )
        ...
        if "metatags" in pagemap_df:
            ...
            for m in pagemap_df["metatags"]:
                try:
                    ...
                    metatag_df = pd.concat(
                        [metatag_df, temp_metatags_df], sort=False
                    )
                except Exception:
                    temp_metatags_df = pd.DataFrame(
                        {"delete_me": None}, index=range(1)
                    )
                    metatag_df = pd.concat(
                        [metatag_df, temp_metatags_df], sort=False
            ...
            for col in metatag_df:
                if col in final_df:
                    metatag_df = metatag_df.rename(
                        columns={col: "metatag_" + col}
                    )
...
def serp_youtube(
    ...
    for i, resp in enumerate(responses):
        ...
        temp_df = pd.concat([snippet_df, id_df, thumb_df], axis=1).assign(
            **page_info
        )
        ...
        result_df = pd.concat(
            [result_df, temp_df], sort=False, ignore_index=True
        )
    ...
    if channel_ids:
        channel_details_df = youtube_channel_details(
            channel_ids=channel_ids, key=key
        )

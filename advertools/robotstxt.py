...
def robotstxt_to_df(robotstxt_url, output_file=None):
    ...
    if isinstance(robotstxt_url, (list, tuple, set, pd.Series)):
        ...
        try:
            ...
            robots_open = urlopen(
                Request(robotstxt_url, headers=headers), timeout=45
            )
            ...
            try:
                ...
                if "last_modified" in df:
                    df["robotstxt_last_modified"] = pd.to_datetime(
                        df["last_modified"]
                    )
        ...
        try:
            df["robotstxt_url"] = (
                [robots_open.url] if df.empty else robots_open.url
            )
    ...
    with futures.ThreadPoolExecutor(max_workers=24) as executor:
        ...
        for future in done_iter:
            ...
            if output_file is not None:
                ...
                final_df = pd.concat(
                    [final_df, future_result], ignore_index=True
                )

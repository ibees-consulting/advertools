            ...
            for line in source_file:
                ...
                try:
                    ...
                    with open(errors_file, "at") as err:
                        ...
                        print(
                            "@@".join([str(linenumber), err_line, str(e)]),
                            file=err,
                        )
                    pass
...
def crawllogs_to_df(logs_file_path):
    ...
    time_middleware_level = (
        "(\d{4}-\d\d-\d\d \d\d:\d\d:\d\d) \[(.*?)\] ([A-Z]+): "
    )
    time_middleware_level_error = (
        "(\d{4}-\d\d-\d\d \d\d:\d\d:\d\d) \[(.*?)\] (ERROR): "
    )
    ...
    redirect_regex = (
        time_middleware_level
        + "(Redirect)ing \((\d\d\d)\) to <([A-Z]+) (.*?)> from <([A-Z]+) (.*?)>"
    )
    redirect_cols = [
        "time",
        "middleware",
    ...
    blocked_regex = (
        time_middleware_level + "(Forbidden) by robots\.txt: <([A-Z]+) (.*?)>"
    )
    blocked_cols = [
        "time",
        "middleware",
    ...
    error_regex = (
        time_middleware_level
        + "Spider (error) processing <([A-Z]+) (.*?)> \(referer: (.*?)\)"
    )
    error_cols = [
        "time",
        "middleware",
    ...
    with open(logs_file_path) as file:
        for line in file:
            ...
            if re.findall(generic_error_regex, line):
                generic_error_lines.append(
                    re.findall(generic_error_regex, line)[0]
                )

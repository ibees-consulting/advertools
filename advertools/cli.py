...
""",
    ...
    def sitemaps(args):
        ...
        if not sitemap_url:
            print(
                "error: please provide a value for sitemap_url", file=sys.stderr
            )
            sys.exit(1)
        sitemap_df = adv.sitemap_to_df(
            sitemap_url, recursive=bool(args.recursive)
        )
    ...
    urls_parser.add_argument(
        "url_list", nargs="*", help="a list of URLs to parse"
    )
...
"buy product" and "product buy" or just "buy product"?""",
    ...
    kwds_parser.add_argument(
        "-n", "--campaign-name", type=str, default="SEM_campaign"
    )
...
word/phrase lengths can also be modified using the --phrase-len option""",
    ...
    emoji_parser.add_argument(
        "regex", type=str, help="pattern to search for emoji"
    )
    ...
    extract_parser.add_argument(
        "entity",
        help="which entity you want to extract",
    ...
    def tokenize(args):
        ...
        if not text_list:
            print(
                "error: please provide a value for text_list", file=sys.stderr
            )
            sys.exit(1)

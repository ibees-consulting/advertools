...
def activities_list(
    ...
    *Optional parameters:*
    ...
    if not set(part.split(",")).issubset(part_params):
        raise ValueError(
            "make sure your `part` parameter is one or more of "
            + str(part_params)
        )
...
def video_categories_list(key, part, id=None, regionCode=None, hl=None):
    ...
    *Optional parameters:*
    ...
    if sum([bool(p) for p in [id, regionCode]]) != 1:
        raise ValueError(
            "make sure you specify exactly one of ['id', 'regionCode']"
        )
...
def subscriptions_list(
    ...
    *Optional parameters:*
    ...
    if (
        ...
        raise ValueError(
            "make sure you specify exactly one of ['channelId', 'id', 'mine', 'myRecentSubscribers', 'mySubscribers']"
        )
...
def playlist_items_list(
    ...
    *Optional parameters:*
    ...
    if sum([bool(p) for p in [id, playlistId]]) != 1:
        raise ValueError(
            "make sure you specify exactly one of ['id', 'playlistId']"
        )
...
def guide_categories_list(key, part, id=None, regionCode=None, hl=None):
    ...
    *Optional parameters:*
    ...
    # return _combine_requests(args, base_url, count=None, max_allowed=None)
    raise ValueError(
        "This function has been deprecated as of September 9, 2020"
    )
...
def comment_threads_list(
    ...
    *Optional parameters:*
    ...
    if (
        ...
        raise ValueError(
            "make sure you specify exactly one of ['allThreadsRelatedToChannelId', 'channelId', 'id', 'videoId']"
        )
...
def comments_list(
    ...
    *Optional parameters:*
    ...
    if sum([bool(p) for p in [id, parentId]]) != 1:
        raise ValueError(
            "make sure you specify exactly one of ['id', 'parentId']"
        )
...
def channels_list(
    ...
    *Optional parameters:*
    ...
    if (
        ...
        raise ValueError(
            "make sure you specify exactly one of ['categoryId', 'forUsername', 'id', 'managedByMe', 'mine', 'mySubscribers']"
        )

...
def _expand_entities(df):
    if "tweet_entities" in df:
        ...
        hashtags = [
            ", ".join(["#" + x["text"] for x in y])
            for y in entities_df["hashtags"]
        ]
        urls = [
            ", ".join([x["expanded_url"] for x in y])
...
def make_dataframe(func):
    ...
    def wrapper(count=None, max_id=None, *args, **kwargs):
        nonlocal func
        # Need review if "twtr" should be replaced?
        ...
        for i, count in enumerate(counts):
            ...
            resp = func(
                count=count, max_id=max_id, cursor=cursor, *args, **kwargs
            )
        ...
        for resp in responses:
            if SPECIAL_KEY_FUNCS.get(fname):
                ...
                if fname in USER_DATA_EMBEDDED:
                    ...
                    temp_df = resp_df
            final_df = pd.concat(
                [final_df, temp_df], sort=False, ignore_index=True
            )
        ...
        for col in final_df:
            if "source" in col:
                ...
                final_df[col] = final_df[col].str.extract('nofollow">(.*)</a>')[
                    0
                ]
...
def get_list_subscriptions(
    user_id=None, screen_name=None, count=None, cursor=None
):
    """
...
@authenticate
def lookup_user(
    screen_name=None, user_id=None, include_entities=None, tweet_mode=None
):
    """

...
def test_set_auth_params():
    ...
    for func in FUNCTIONS:
        ...
        assert (
            func.get_auth_params()["oauth_token_secret"] == oauth_token_secret
        )
...
def test_get_favorites():
    result = get_favorites(
        screen_name="twitter", count=5, tweet_mode="extended"
    )
    assert type(result) == pd.core.frame.DataFrame
    assert len(result) <= 5
...
def test_get_followers_list():
    result = get_followers_list(
        screen_name="eliasdabbas", count=5, skip_status=None
    )
    assert type(result) == pd.core.frame.DataFrame
    assert len(result) <= 5
...
def test_get_list_members():
    result = get_list_members(
        slug="nyt-journalists", owner_screen_name="nytimes", count=5
    )
    assert type(result) == pd.core.frame.DataFrame
    assert len(result) <= 5
    assert "screen_name" in result
...
def test_get_list_statuses():
    result = get_list_statuses(
        slug="nyt-journalists",
        owner_screen_name="nytimes",
        count=5,
        tweet_mode="extended",
    )
    assert type(result) == pd.core.frame.DataFrame
...
def test_get_retweets():
    result = get_retweets(
        id=1032676609902428160, tweet_mode="extended", count=5
    )
    assert type(result) == pd.core.frame.DataFrame
    assert len(result) <= 5
    assert all(
...
def test_get_user_timeline():
    result = get_user_timeline(
        screen_name="bbc", count=5, tweet_mode="extended"
    )
    assert type(result) == pd.core.frame.DataFrame
    assert len(result) <= 5
    assert "tweet_full_text" in result
    ...
    long_result = get_user_timeline(
        screen_name="eliasdabbas", count=2000, tweet_mode="extended"
    )
    assert type(result) == pd.core.frame.DataFrame
...
def test_lookup_status():
    result = lookup_status(
        id=[1032676609902428160, 1031574983095533568], tweet_mode="extended"
    )
    assert type(result) == pd.core.frame.DataFrame
    assert len(result) <= 5
    assert all(
...
def test_search():
    ...
    result_compat = search(
        q="basketball", lang="en", count=5, tweet_mode="compat"
    )
    assert type(result) == pd.core.frame.DataFrame

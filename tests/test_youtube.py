...
def test_captions_list():
    result = yt.captions_list(
        key=youtube_key, part="snippet", videoId="kJQP7kiw5Fk"
    )
    assert {"queryTime", "param_part"}.issubset(result.columns)
...
def test_video_categories_list():
    result = yt.video_categories_list(
        key=youtube_key, part="snippet", regionCode="de"
    )
    assert {"queryTime", "param_part"}.issubset(result.columns)
...
def test_with_zero_results_required():
    result = yt.search(
        key=youtube_key, part="snippet", q="testing bitcoin", maxResults=0
    )
...
def test_empty_list_raises_error():
    with pytest.raises(ValueError):
        yt.search(
            key=youtube_key, part="snippet", q="testing bitcoin", regionCode=[]
        )

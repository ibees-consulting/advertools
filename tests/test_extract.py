...
summary_flats = {
    k: [d[key] for key in d if "flat" in key and "emoji_flat_text" not in key][
        0
    ]
...
summary_tops = {
    k: [d[key] for key in d if "top" in key] for k, d in summaries.items()
}
...
def make_hashable(iterable):
    ...
    return tuple(
        tuple(sorted(x)) if isinstance(x, list) else x for x in iterable
    )
...
summary_overviews = {
    k: [d[key] for key in d if "overview" in key][0]
    for k, d in summaries.items()
}

...
dont_follow_url_params_df = pd.read_json(
    "dont_follow_url_params.jl", lines=True
)


def test_dont_follow_url_params_not_followed():
    assert (
        not dont_follow_url_params_df["url"]
...
def test_image_tags_available():
    assert [
        col in crawl_df
        for col in ["img_src", "img_alt", "img_height", "img_width"]
    ]
...
broken_links_file = os.path.abspath(
    "tests/data/crawl_testing/broken_links.html"
)

crawl(
    ["file://" + broken_links_file, "wrong_url"],

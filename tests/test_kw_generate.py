...
class KeywordTests(unittest.TestCase):
    def test_match_types_are_capitalized(self):
        ...
        self.assertEqual(
            set(df["Criterion Type"]), {"Exact", "Phrase", "Broad"}
        )
    ...
    def test_right_combinatorics_fuction_selected(self):
        df_perm = kw_generate(
            ["one", "two"], ["three", "four"], order_matters=True
        )
        self.assertTrue(
            {"one three", "three one"}.issubset(set(df_perm["Keyword"]))
        )
        df_comb = kw_generate(
            ["one", "two"], ["three", "four"], order_matters=False
        )
        self.assertFalse(
            {"one three", "three one"}.issubset(set(df_comb["Keyword"]))
        )
    ...
    def test_correct_combinations_are_generated(self):
        ...
        df = kw_generate(
            products, words, match_types=["Exact"], order_matters=False
        )
    ...
    def test_correct_permutations_are_generated(self):
        ...
        df = kw_generate(
            products, words, match_types=["Exact"], order_matters=True
        )
        self.assertEqual(set(test_combs), set(df["Keyword"]))
